...
func TryOut(player Player) {
	...
	//recorder := gadget.TapeRecorder(player) // cannot convert player (variable of type Player) to type gadget.TapeRecorder:
	//											need type assertion
	recorder := player.(gadget.TapeRecorder) // ASSERTING the Player is a TapeRecorder
...
func main() {
	...
	TryOut(player2) // panic: interface conversion: main.Player is gadget.TapePlayer, not gadget.TapeRecorder
}
