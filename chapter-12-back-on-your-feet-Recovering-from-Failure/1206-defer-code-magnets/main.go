...
func find(item string, slice []string) bool {
	for _, sliceItem := range slice {
		if item == sliceItem {
			return true // Return true of the string is found in the slice...
		}
	}
	return false // ... or false if the string is not found.
}

// Refrigerator type is based on a slice of strings,
// which will hold the names of the foods the refrigerator contains.
